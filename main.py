...
SEARCH_QUERY = "select gw_num, is_seed, name, rank, points, id from rankings where id in (select id from rankings where id in (select id from rankings where name like ? escape '!') and gw_num = (select max(gw_num) from rankings)) order by id, gw_num desc"
...
def response_helper(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        return json.dumps(obj) if isinstance(obj, dict) else obj, return_code, {'Content-Type': 'application/json; charset=utf-8'}
...
def upload():
    ...
    try:
        ...
        copy(DB_FILE, '{}.{}'.format(DB_FILE, int(time())))
        conn.commit()
